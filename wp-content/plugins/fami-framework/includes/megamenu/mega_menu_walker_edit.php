...
class Mega_Menu_Walker_Edit extends Walker_Nav_Menu_Edit {
    ...
    public function start_el( &$output, $item, $depth = 0, $args = array(), $id = 0 ){
        ...
        $default_mega_menu_setting = fmfw_get_default_setting_of_mega_menu();
        $mega = get_post_meta( $item->ID, '_fm_menu_item_mega', true );
        $mega = wp_parse_args( $mega, $default_mega_menu_setting );
        $mega_content = $mega['content'];
        unset($mega['content']);
        ...
        if ( !empty( $item->_invalid ) ) {
            ...
            $title = sprintf( __( '%s (Invalid)','fami-framework' ), $item->title );
        } elseif ( isset( $item->post_status ) && 'draft' == $item->post_status ) {
            $classes[] = 'pending';
            /* translators: %s: title of menu item in draft status */
            $title = sprintf( __( '%s (Pending)' ,'fami-framework'), $item->title );
        }
        ...
        <div class="menu-item-bar">
            <div class="menu-item-handle">
                    <span class="item-title">
                        ...
                        <span class="is-submenu" <?php echo $submenu_text; ?>><?php _e( 'sub item','fami-framework' ); ?></span>
                        <span id="fmfw_mega_menu_item_settings-<?php echo esc_attr( $item_id ); ?>"
                              ...
                              class="fmfw_mega_menu_settings "> <?php esc_html_e( 'Settings', 'fami-framework' ); ?>
                        </span>
                        <span class="hidden mega-data" data-mega="<?php echo esc_attr(json_encode( $mega ));?>"><?php echo $mega_content;?></span>
                    </span>
                <span class="item-controls">
						...
						<span class="item-order hide-if-js">
                            ...
                            ?>
			    " class="item-move-up" aria-label="<?php esc_attr_e( 'Move up','fami-framework' ) ?>">&#8593;</a>
							|
							<a href="
							<?php
                            echo wp_nonce_url(
                            ...
                            ?>
			    " class="item-move-down" aria-label="<?php esc_attr_e( 'Move down','fami-framework' ) ?>">&#8595;</a>
						</span>
						<a class="item-edit" id="edit-<?php echo $item_id; ?>" href="<?php
                        echo ( isset( $_GET['edit-menu-item'] ) && $item_id == $_GET['edit-menu-item'] ) ? admin_url( 'nav-menus.php' ) : add_query_arg( 'edit-menu-item', $item_id, remove_query_arg( $removed_args, admin_url( 'nav-menus.php#menu-item-settings-' . $item_id ) ) );
                        ?>
			" aria-label="<?php esc_attr_e( 'Edit menu item','fami-framework' ); ?>"><span
                                class="screen-reader-text"><?php _e( 'Edit' ,'fami-framework'); ?></span></a>
					</span>
        ...
        <div class="menu-item-settings wp-clearfix" id="menu-item-settings-<?php echo $item_id; ?>">
            ...
            <p class="description description-wide">
                <label for="edit-menu-item-title-<?php echo $item_id; ?>">
                    <?php _e( 'Navigation Label','fami-framework' ); ?><br/>
            ...
            <p class="field-title-attribute field-attr-title description description-wide">
                <label for="edit-menu-item-attr-title-<?php echo $item_id; ?>">
                    <?php _e( 'Title Attribute','fami-framework' ); ?><br/>
            ...
            <p class="field-link-target description">
                <label for="edit-menu-item-target-<?php echo $item_id; ?>">
                    ...
                    <?php _e( 'Open link in a new tab','fami-framework' ); ?>
                </label>
            </p>
            <p class="field-css-classes description description-thin">
                <label for="edit-menu-item-classes-<?php echo $item_id; ?>">
                    <?php _e( 'CSS Classes (optional)','fami-framework' ); ?><br/>
            ...
            <p class="field-xfn description description-thin">
                <label for="edit-menu-item-xfn-<?php echo $item_id; ?>">
                    <?php _e( 'Link Relationship (XFN)','fami-framework' ); ?><br/>
            ...
            <p class="field-description description description-wide">
                <label for="edit-menu-item-description-<?php echo $item_id; ?>">
                    <?php _e( 'Description','fami-framework' ); ?><br/>
                    <textarea id="edit-menu-item-description-<?php echo $item_id; ?>"
                        ...
                        ?></textarea>
                    <span class="description"><?php _e( 'The description will be displayed in the menu if the current theme supports it.','fami-framework' ); ?></span>
                </label>
            </p>

            <fieldset class="field-move hide-if-no-js description description-wide">
                <span class="field-move-visual-label" aria-hidden="true"><?php _e( 'Move','fami-framework' ); ?></span>
                <button type="button" class="button-link menus-move menus-move-up"
                        data-dir="up"><?php _e( 'Up one' ,'fami-framework'); ?></button>
                <button type="button" class="button-link menus-move menus-move-down"
                        data-dir="down"><?php _e( 'Down one','fami-framework' ); ?></button>
                <button type="button" class="button-link menus-move menus-move-left" data-dir="left"></button>
                <button type="button" class="button-link menus-move menus-move-right" data-dir="right"></button>
                <button type="button" class="button-link menus-move menus-move-top"
                        data-dir="top"><?php _e( 'To the top' ,'fami-framework'); ?></button>
            </fieldset>

            <div class="menu-item-actions description-wide submitbox">
                <?php if ( 'custom' != $item->type && $original_title !== false ) : ?>
                    <p class="link-to-original">
                        <?php printf( __( 'Original: %s','fami-framework' ), '<a href="' . esc_attr( $item->url ) . '">' . esc_html( $original_title ) . '</a>' ); ?>
                    </p>
                <?php endif; ?>
                ...
                ); ?>"><?php _e( 'Remove' ,'fami-framework'); ?></a> <span class="meta-sep hide-if-no-js"> | </span> <a
			...
			<?php
                    ...
                    ?>
			#menu-item-settings-<?php echo $item_id; ?>"><?php _e( 'Cancel' ,'fami-framework'); ?></a>
            </div>
