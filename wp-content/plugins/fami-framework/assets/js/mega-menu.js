...
(function ( $, _ ) {
    ...
    fmm = {
        init: function () {
            ...
            this.itemData = {};
            this.templates = {
                menus   : wp.template('fmfw-menus'),
                title   : wp.template('fmfw-title'),
                mega   : wp.template('fmfw-mega'),
                background   : wp.template('fmfw-background'),
                icon   : wp.template('fmfw-icon'),
                content   : wp.template('fmfw-content'),
                design   : wp.template('fmfw-design'),
                badges   : wp.template('fmfw-badges'),
                settings   : wp.template('fmfw-settings')
            };
        ...
        initActions: function () {
            ...
            fmm.$modal
                ...
                .on( 'change', '.fmm-mega-enable-field select', this.toggleEnableMega )
                .on( 'change', '.fmm-mega-width-field select', this.toggleWidthField )
                .on( 'change', '.fmm-mega-block-field select', this.toggleContentField );
        },
        ...
        toggleEnableMega: function() {
            var $el = $( this );
            if ( parseInt($el.val()) > 0) {
                var mega_width = $el.closest( '.setting-field' ).siblings('.fmm-mega-width-field');
                if (mega_width.find('select').val() == 'custom') {
                    $el.closest( '.setting-field' ).siblings('.setting-field').show();
                } else {
                    mega_width.show();
                }
                var mega_post = $el.closest('.mega-settings').siblings('.mega-settings');
                if (parseInt(mega_post.find('.fmm-mega-block-field select').val()) > 0) {
                    mega_post.show();
                } else {
                    mega_post.show();
                    $('#fmm-mega-content').show();
                }

            } else {
                $el.closest( '.setting-field' ).siblings('.setting-field').hide();
                $el.closest('.mega-settings').siblings('.mega-settings').hide();
                $('#fmm-mega-content').hide();
            }
        },
        toggleWidthField: function() {
            var $el = $( this );
            if ( 'custom' === $el.val() ) {
                $el.closest( '.setting-field' ).next( '.setting-field' ).show();
            } else {
                $el.closest( '.setting-field' ).next( '.setting-field' ).hide();
            }
        },
        toggleContentField: function() {
            var $el = $( this );
            if ( parseInt($el.val()) > 0) {
                $('#fmm-mega-content').hide();
            } else {
        ...
        initColorFields: function () {
            fmm.$modal.find( '.badge-color-picker' ).wpColorPicker();
        },
        ...
        initIconFields: function () {
            // Background image
            fmm.$modal.on( 'click', '.icon-image .upload-button', function ( e ) {
                e.preventDefault();
                ...
                fmm.frame.on( 'select', function () {
                    ...
                    $el.siblings( '.icon-preview' ).html( '<img src="' + url + '">' );
                    $el.siblings( 'input' ).val( url );
                    $el.siblings( '.remove-button' ).removeClass( 'hidden' );
                } );

                fmm.frame.open();
            } ).on( 'click', '.icon-image .remove-button', function ( e ) {
                e.preventDefault();

                var $el = $( this );

                $el.siblings( '.icon-preview' ).html( '' );
                $el.siblings( 'input' ).val( '' );
                $el.addClass( 'hidden' );
            } );
        },
